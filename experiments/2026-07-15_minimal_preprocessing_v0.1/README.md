# Minimal Wearable EEG Preprocessing Validation v0.1

**Work date:** 2026-07-15
**Dataset:** BOAS `ds005555`, snapshot `1.1.1`
**Specification:** `docs/preprocessing/minimal_wearable_eeg_preprocessing_spec_v0.1.md`
...
Fail the mechanical preprocessing validation. Preserve the failed rows and revise the input-quality or preprocessing rule before the gate.

The failed rows are `T0120` and `T0196`. Both contain finite data but lack the required 240-second coverage. The resolution is recorded in signal-quality v0.3 and preprocessing v0.2.

## 5. Outputs

| File | Purpose |
|---|---|
| `filter_response_v0.1.tsv` | Measured single-pass and forward-backward filter response |
| `synthetic_frequency_checks_v0.1.tsv` | Predeclared retain/attenuate checks |
| `train_robust_scaler_v0.1.tsv` | Scaling fitted from train recordings only |
| `train_recording_preprocessing_checks_v0.1.tsv` | Continuous-recording integrity checks |
| `train_window_preprocessing_checks_v0.1.tsv` | Compact raw, filtered, and normalized window summaries |
| `train_window_preprocessing_summary_v0.1.tsv` | Counts by window and quality class |
