...
Additional evidence completed on 2026-09-11 to 2026-09-13:

- predeclared a validation-only single-channel feature-contribution experiment before implementation or execution;
- retained the frozen `H2-D` model, scaler, threshold 0.96, validation membership, temporal support, and event-matching rules without refitting;
- neutralized exactly 40 context dimensions for `HB_1` or `HB_2` using the corresponding train-fitted model means;
- reproduced the intact Block 7 probabilities within `1.12e-16` and all four intact event summaries within `1e-12`;
- found that `HB_1` neutralization eliminated all true detections, reducing F1 from 0.1123 to 0 while reducing false alarms by 0.9350/hour;
- found that `HB_2` neutralization reduced F1 by 0.0210 and false alarms by 0.3765/hour, remaining inside the predefined material bounds;
- confirmed both F1 and false-alarm directions in all 16 leave-one-`pid`-out folds for each ablation;
...
- predeclared one train-only Gaussian-noise augmentation mechanism before implementation and result generation;
- generated 328 train noise-feature arrays with 492/492 SNR calibrations passing and exact five-condition multiplicity for 2,743 base labeled candidates;
- selected threshold 0.99 from 75,539 full-night scores produced by five participant-grouped train folds, then froze the final model before validation;
- reduced the isolated `HB_1` noise false-alarm result from 4.4616 to 0.9413/hour and improved F1 from 0.0419 to 0.0918;
- found that the `HB_1` FAR reduction interval excluded zero while its F1 improvement interval crossed zero;
- failed the meaningful clean-advancement gate because clean F1 decreased from 0.1123 to 0.1024 rather than improving by at least 0.05;
...
- predeclared three fixed alarm-fusion rules before calculating their combined false-alarm burden;
- reproduced all 12 frozen direct-model metric rows and clustered each applicable source alarm exactly once under a fixed 30-second maximum span;
- found that `P6-H2-OR` increased recall but reduced F1 from 0.1631 to 0.1100 and increased false alarms from 1.1107 to 2.2465/hour;
- found the same adverse direction for all-direct OR, with F1 and FAR participant intervals excluding zero;
- found that 2-of-3 laboratory consensus reduced 210 `P6-D` alarms to 110, producing F1 0.2174 and 0.5397 false alarms/hour;
- retained the consensus F1 gain as participant-inconclusive because its difference interval was -0.0192 to +0.1323, while the FAR reduction interval excluded zero;
- found that the consensus +/-45-second F1 gain was positive but failed the predeclared +0.05 materiality gate;
...
Standalone temporal-representation screen completed on 2026-09-22:

- predeclared one bidirectional LSTM plus linear-chain CRF and a matched flattened logistic comparator before implementation;
- reused the same 2,743 reviewed train candidates, eight-epoch wearable features, five participant folds, event evaluator, and closed validation/test boundary;
- obtained primary train-OOF F1 0.1965 and 0.5306 false alarms/hour for `LC-1`, versus 0.0982 and 1.4679/hour for `LR-OOF`;
- passed the aggregate +0.05 F1 and no-FAR-increase gate, with paired difference intervals of +0.0485 to +0.1367 F1 and -1.4804 to -0.4089 false alarms/hour;
- retained the full material-size finding as point-supported because the F1 interval lower bound was slightly below +0.05;
- preserved a first-run path-check failure and corrected only the over-broad predicate;
