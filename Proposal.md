...
**Descriptive test update, 2026-09-06:** The four frozen comparators were applied once to 26 test recordings from 20 `pid` groups with exact model hashes and unchanged thresholds. `P6-D` obtained F1 0.2048 and 0.4525 false alarms per hour, `P2-D` 0.1429 and 0.3570, `H2-D` 0.1493 and 1.2621, and `P2-H2-Z` 0.1067 and 0.4173. The `P6-D - P2-D` F1 difference was +0.0620 with paired 95% interval +0.0019 to +0.1194, reversing the validation point direction; its false-alarm interval crossed zero. The `P2-D - P2-H2-Z` F1 difference remained positive at +0.0362 but its interval crossed zero. Relative to direct wearable fitting, zero-shot transfer again traded lower F1 for fewer false alarms, with a false-alarm difference of -0.8448/hour and interval -1.2441 to -0.5081. The fixed quality sensitivity preserved six-channel PSG as the leading comparator. All 13/13 in-run and 13/13 independent checks passed, including exact reconstruction of 95,460 probabilities. Block 7 is complete, but these reused-test results remain descriptive and cannot support post-test revision or independent confirmation.
...
**Raw-signal noise update, 2026-09-11:** A separately predeclared validation-only experiment propagated deterministic raw-rate Gaussian noise through the frozen preprocessing path and evaluated five fixed perturbations without refitting. The 20 dB both-channel condition passed the mild-noise bounds, although its +0.0287 F1 change had a paired interval crossing zero. Probability rank correlation fell from 0.8404 at 20 dB to 0.7276 at 10 dB and 0.4982 at 0 dB, while mean absolute probability difference increased from 0.0705 to 0.1260 to 0.1640, supporting the nested dose hypothesis. At 0 dB no true event was detected. Isolated 10 dB `HB_1` noise reduced F1 by 0.0704 and increased false alarms by 3.0058/hour, with both participant intervals excluding zero; `HB_2` noise caused a smaller F1 loss and reduced alarms. All 160 SNR calibrations, 17/17 in-run checks, and 12/12 independent checks passed; the independent reconstruction was repeated twice and the immutable result rerun produced no change. This controlled white-noise result does not establish robustness to natural wearable artefacts or authorize threshold revision.

**Train-only augmentation update, 2026-09-13:** One predeclared adaptation retained the existing bandpower-logistic pipeline while exposing every labeled train candidate to clean and four fixed Gaussian-noise conditions. Five participant-grouped train folds selected threshold 0.99 before validation. On clean validation, `H2-NA` produced F1 0.1024 and 1.2801 false alarms/hour versus `H2-D` F1 0.1123 and 1.4558/hour. Under isolated 10 dB `HB_1` noise, `H2-NA` improved F1 from 0.0419 to 0.0918 and reduced false alarms from 4.4616 to 0.9413/hour; the FAR reduction interval excluded zero but the F1 interval crossed zero. The targeted robustness gates passed, but the required +0.05 clean-F1 advancement failed and other noise conditions showed FAR tradeoffs. The generic augmentation path therefore stops at v0.1 and the model is retained only as a robustness comparator. All 13/13 train, 11/11 validation, and 14/14 independent checks passed, with the independent reconstruction completed twice and no current-test access.

**Cross-modality event-overlap update, 2026-09-13:** A separately predeclared analysis used only frozen validation event matches to determine whether direct PSG and wearable models detected the same references. At +/-15 seconds, 13/37 primary references were detected by both direct PSG and direct wearable, seven by PSG only, three by wearable only, and 14 by neither. The non-deployable direct-model union recall was 0.6216, 0.1081 above the best individual direct-model recall, but its false-alarm burden was not evaluated and its participant interval included zero. Expanding tolerance to +/-45 seconds increased union recall by 0.1081 with a participant interval of 0.0238 to 0.2400. This makes stage-derived boundary uncertainty a material next question while retaining wearable-specific information loss and low precision as unresolved limitations. No fitting, threshold selection, or current-test access occurred; 9/9 in-run and 10/10 independent checks passed.
...
**Participant evidence-strength update, 2026-09-18:** A second predeclared frozen-output analysis graded 25 original Block 8 claims against their stored participant bootstrap intervals or leave-one-`pid`-out ranges. Eight passing gates were supported by their full range, eight were point-gate-only, four failures were supported by their full range, four failed points had ranges overlapping the gate, and one joint difference-in-differences claim had no direct interval. The clean augmentation `+0.05` F1 advancement failure was interval-supported because its range was -0.0438 to +0.0132. The boundary-tolerance range was entirely positive at +0.0238 to +0.2400 but did not clear the original `+0.10` materiality gate. Boundary uncertainty therefore remains prioritized, while material boundary gain is not described as participant-supported. All 11/11 primary and 10/10 independent checks passed, and the immutable rerun changed no file.
